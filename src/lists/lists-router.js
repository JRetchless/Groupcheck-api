...
listsRouter.use(function(req,res,next){ if(req.session.user) { next() } else { 
  
  console.dir('req session failed, req session data')
...
.route('/')
.get((req, res) => {
  console.dir('listRouter .get req.session')
  console.dir(req.session)
